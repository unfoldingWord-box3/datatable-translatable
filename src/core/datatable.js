...
export const cellEdit = ({
  ...
  if ( rowIndex >= rows.length ) {
    ...
    console.log("Copy From Source Row:", data[rowIndex])
    
    for (let i=0; i < _rows[rowIndex].length; i++) {
      //console.log("copying: from, to", rowIndex, i, " values:", _data[rowIndex][i], _rows[rowIndex][i])
      //_rows[rowIndex][i] = _data[rowIndex+1][i];
    }
